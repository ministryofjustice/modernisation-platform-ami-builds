resource "aws_imagebuilder_image_pipeline" "windowsserver2012" {
  ...
  name                             = local.windows_2012_pipeline.pipeline.name

  schedule {
    schedule_expression                = local.windows_2012_pipeline.pipeline.schedule
...
resource "aws_imagebuilder_image_recipe" "windowsserver2012" {
  block_device_mapping {
    device_name = local.windows_2012_pipeline.recipe.device_name

    ebs {
      delete_on_termination = local.windows_2012_pipeline.recipe.ebs.delete_on_termination
      volume_size           = local.windows_2012_pipeline.recipe.ebs.volume_size
      volume_type           = local.windows_2012_pipeline.recipe.ebs.volume_type
      encrypted             = local.windows_2012_pipeline.recipe.ebs.encrypted
      kms_key_id            = data.aws_kms_key.ebs_encryption_cmk.arn
    }
  }

  dynamic "component" {
    for_each = toset(local.windows_2012_pipeline.components)
    content {
  ...
  dynamic "component" {
    for_each = toset(local.windows_2012_pipeline.aws_components)
    content {
  ...
  name         = local.windows_2012_pipeline.recipe.name
  parent_image = local.windows_2012_pipeline.recipe.parent_image
  version      = local.windows_2012_pipeline.recipe.version
}


resource "aws_imagebuilder_infrastructure_configuration" "windowsserver2012" {
  description                   = local.windows_2012_pipeline.infra_config.description
  instance_profile_name         = aws_iam_instance_profile.image_builder_profile.name
  instance_types                = local.windows_2012_pipeline.infra_config.instance_types
  name                          = local.windows_2012_pipeline.infra_config.name
  security_group_ids            = local.windows_2012_pipeline.infra_config.security_group_ids
  subnet_id                     = local.windows_2012_pipeline.infra_config.subnet_id
  terminate_instance_on_failure = local.windows_2012_pipeline.infra_config.terminate_on_fail
...
resource "aws_imagebuilder_component" "windowsserver2012_components" {
  for_each = { for file in local.windows_2012_pipeline.components : file => yamldecode(file("components/windows/${file}")) }
...
resource "aws_imagebuilder_distribution_configuration" "windowsserver2012" {
  name = local.windows_2012_pipeline.distribution.name

  distribution {
    region = local.windows_2012_pipeline.distribution.region

    ami_distribution_configuration {

      name       = local.windows_2012_pipeline.distribution.ami_name
      kms_key_id = data.aws_kms_key.ebs_encryption_cmk.arn
