...
Terraform for managing onr AMI pipelines and images.
...
In the relevant development environment, e.g. `onr-development`,
...
contains the development account name, e.g. `onr-development`.
...
- manually run the `onr` pipeline. First plan, and then apply.
